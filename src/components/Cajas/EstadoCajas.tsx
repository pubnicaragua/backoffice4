...
import { Badge, Card, Table, Button, Alert, Spinner, Select } from 'flowbite-react';
...
interface SesionCajaCompleta extends Omit<SesionCaja, 'usuario_id' | 'caja_id'> {
...
export function EstadoCajas({ sucursalId, modoCompacto = false }: EstadoCajasProps) {
    ...
    return (
      <Card className="mb-4">
        <div className="flex justify-between items-center mb-4">
          ...
          <Button size="xs" color="light" onClick={cargarSesiones} disabled={refrescando}>
            <HiRefresh className={`mr-2 h-4 w-4 ${refrescando ? 'animate-spin' : ''}`} />
        ...
        {cargando ? (
          ...
          <div className="space-y-2">
            {sesiones.length === 0 ? (
              <p className="text-gray-500 text-center py-4">No hay cajas abiertas</p>
            ) : (
              sesiones
                .filter(s => s.estado === 'abierta')
                .map(sesion => (
                  <div key={sesion.id} className="border rounded-lg p-3 flex justify-between items-center">
                    <div>
                      <p className="font-medium">{sesion.caja.nombre}</p>
                      <p className="text-sm text-gray-600">
                        {sesion.usuario.user_metadata?.nombre_completo || sesion.usuario.email}
                      </p>
                    </div>
          ...
          <Table hoverable>
            ...
            <Table.Body className="divide-y">
              {sesiones.length === 0 ? (
                ...
                sesiones.map(sesion => (
                  <Table.Row key={sesion.id} className="bg-white dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <Table.Cell>
                      <div>
                        <p className="font-medium">
                          {sesion.usuario.user_metadata?.nombre_completo || 'Usuario sin nombre'}
                        </p>
                        <p className="text-xs text-gray-500">{sesion.usuario.email}</p>
                      </div>
                    </Table.Cell>
                    <Table.Cell>
                      <Badge
                    ...
                    <Table.Cell>
                      {formatearFecha(sesion.abierta_en)}
                    </Table.Cell>
                    <Table.Cell>
                      {sesion.estado === 'abierta' && (
                        <Button 
