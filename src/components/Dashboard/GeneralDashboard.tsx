    ...
    <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-300">  
        ...
        {chartData.map((item, index) => (  
          <div  
            ...
            className={`flex items-center justify-between text-sm p-2 rounded-lg transition-all duration-200 ${  
              hoveredSlice === index ? 'bg-gray-50 scale-105' : 'hover:bg-gray-50'  
            }`}  
            onMouseEnter={() => setHoveredSlice(index)}  
            onMouseLeave={() => setHoveredSlice(null)}  
          >  
            <div className="flex items-center space-x-3">  
              <div  
                className={`w-4 h-4 rounded-full transition-all duration-200 ${  
                  hoveredSlice === index ? 'scale-125 shadow-md' : ''  
                }`}  
      ...
      {!hasData && (  
        <div className="mt-4 text-center p-4 bg-gray-50 rounded-lg">  
          <p className="text-sm text-gray-500 mb-2">No hay datos disponibles para mostrar</p>  
          <p className="text-xs text-gray-400">Los datos aparecerán aquí cuando estén disponibles</p>  
        </div>  
      )}  
...
export default function GeneralDashboard() {  
  ...
  const processLossData = () => {  
    ...
    if (robo === 0 && vencimiento === 0 && dano === 0 && otro === 0) {  
      return [{ name: "Sin mermas reportadas", value: 1, color: "#E5E7EB" }];  
    }  
