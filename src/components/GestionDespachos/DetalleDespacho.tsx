...
export function DetalleDespacho({ onBack, despacho, isOpen }: DetalleDespachoProps) {
  ...
  useEffect(() => {
    if (despacho?.folio_factura) {
      loadProductos();
    }
  }, [despacho?.folio_factura]);

  const loadProductos = useCallback(async () => {
    console.log("🚀 loadProductos llamado con folio:", despacho?.folio_factura);

    if (!despacho?.folio_factura) return;
    ...
    ventas!inner ( id, folio )
  `)
      .eq("ventas.folio", despacho.folio_factura);
        ...
        <body>
          <div class="header">
            <div class="company-info">
              <h2>${userEmpresa?.razon_social || ""}</h2>
              <p>RUT: ${userEmpresa?.rut || ""}</p>
              <p>${userEmpresa?.direccion + "," || ""} ${userEmpresa?.comuna || ""}</p>
              <p>${userEmpresa?.telefono || ""}</p>
            </div>
          </div>
          ...
          <div class="document-info">
            ...
            <p><strong>Fecha:</strong> ${despacho?.fecha
          ? new Date(despacho.fecha).toLocaleDateString('es-CL')
                ...
                {productos.map((p, index) => (
                  <div key={index} className="px-6 py-4">
                    <div className="grid grid-cols-4 gap-4 text-sm">
                      <span className="text-gray-900">{p.productos?.nombre}</span>
                      <span className="text-gray-600">{p.productos?.codigo}</span>
                      <span className="text-gray-600">{p.cantidad}</span>
                      <span className="text-gray-900 font-medium">${p.precio_unitario}</span>
                    </div>
                  </div>
                ))}
              </div>
                ...
                <div className="space-y-1 mb-2">
                  {productos.slice(0, 3).map((producto, index) => (
                    <div key={index} className="flex justify-between">
                      ...
                      <span>{producto.cantidad} x ${producto.precio_unitario.toLocaleString("es-CL")}</span>
                    </div>
                  ))}
                  {productos.length > 3 && (
