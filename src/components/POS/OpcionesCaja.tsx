...
export function OpcionesCaja() {
  ...
  const handleSaveConfiguration = async () => {
    if (!filters.sucursal || filters.cajas.length === 0) {
      toast.error(
        "❌ Debes seleccionar una sucursal y al menos una caja antes de guardar"
      );
      return;
    }

    if (!hasPermission(PERMISOS.ConfiguracionSistema)) {
      toast.error(
        "No cuentas con permisos para modificar la configuracion de las cajas"
      );
      return;
    }
    ...
    try {
      ...
      toast.success(
        `✅ Configuración guardada para las cajas: ${cajasNombres}`
      );
    } catch (error) {
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-medium text-gray-900">Opciones de caja</h2>
        <button
      ...
      <div className="space-y-4">
        <h3 className="font-medium text-gray-900">
          ¿Solicitar autorización para eliminar productos de una venta o
          cancelar venta?
        </h3>
        <div className="flex space-x-6">
          <label className="flex items-center space-x-2">
            <input
              ...
              onChange={() =>
                handleSettingChange("solicitar_autorizacion", true)
              }
          ...
          <label className="flex items-center space-x-2">
            <input
              ...
              checked={!settings.solicitar_autorizacion}
              onChange={() =>
                handleSettingChange("solicitar_autorizacion", false)
              }
              className="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500"
            />
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                const sucursal =
                  sucursales?.find((s) => s.id === e.target.value) || null;
          ...
          <div>
            ...
            {filters.sucursal &&
              cajas?.filter((c) => c.sucursal_id === filters.sucursal?.id)
                .length === 0 && (
