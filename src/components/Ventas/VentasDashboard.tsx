...
interface KpiCardProps { // Renamed to KpiCardProps
  ...
  return (
    <div className="bg-gray-50 p-4 rounded-2xl flex flex-col justify-between shadow-sm"> {/* Updated styling */}
      <div className="flex items-center justify-between">
        <div>
          <span className="text-sm font-medium text-gray-700">{title}</span> {/* Updated styling */}
...
export function VentasDashboard() {
  ...
  const [filters, setFilters] = useState({
    fechaInicio: new Date(Date.now() - 14 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // Default to 14 days ago
  ...
  const [kpiData, setKpiData] = useState({ ventasTotales: 0, margen: 0, unidadesVendidas: 0, numeroVentas: 0, ticketPromedio: 0 });
  const [monthlyChartData, setMonthlyChartData] = useState<any[]>([]);

  const { data: ventas = [], refetch } = useSupabaseData<any>('ventas', '*, sucursales(nombre)'); // Keep refetch for manual refresh
  ...
  const calculateMonthlyChartData = React.useCallback(() => {
    ...
    const dataMap = new Map<string, { mes: string; actual: number; anterior: number }>();
  ...
  }, [calculateKpis, calculateMonthlyChartData, ventas, ventaItems, productos, filters]); // Recalculate when dependencies change
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm space-y-4 flex-1">
          ...
          {loadingChart ? (
            ...
            <ResponsiveContainer width="100%" height={300}>
              <ComposedChart data={monthlyChartData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                ...
                <YAxis
                  ...
                  width={100}
                />
                <Tooltip content={<CustomTooltip formatPrice={formatPrice} />} /> {/* Pass formatPrice to CustomTooltip */}
                {showAnterior && (
                  <Line
                    type="monotone"
                    dataKey="anterior"
            ...
            <div>
              ...
              <div className="space-y-2 text-sm">
                {['Caja N°1', 'Caja N°2', 'Caja N°3', 'Caja N°4'].map(caja => (
                  <label key={caja} htmlFor={`caja-${caja}`} className="flex items-center gap-2 cursor-pointer">
                    <input
                      id={`caja-${caja}`}
                      type="checkbox"
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                onChange={(e) => {
                  if (e.target.checked) {
                    setFilters({
                      fechaInicio: new Date(Date.now() - 14 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
...
const CustomTooltip: React.FC<CustomTooltipProps> = ({ active, payload, label, formatPrice }) => {
