...
import { Caja, SesionCaja, AperturaCajaParams, CierreCajaParams } from '../types/cajas';
 ...
 */
export const obtenerCajasPorSucursal = async (sucursalId: string): Promise<Caja[]> => {
  try {
 ...
 */
export const obtenerSesionesActivas = async (sucursalId: string): Promise<SesionCaja[]> => {
  try {
    const { data, error } = await supabase
      .from('sesiones_caja')
      .select('*')
 ...
 */
export const obtenerSesionActivaUsuario = async (usuarioId: string): Promise<SesionCaja | null> => {
  try {
    const { data, error } = await supabase
      .from('sesiones_caja')
      .select('*')
 ...
 */
export const abrirCaja = async (params: AperturaCajaParams): Promise<SesionCaja> => {
  try {
 ...
 */
export const cerrarCaja = async (params: CierreCajaParams): Promise<SesionCaja> => {
  try {
    const { data, error } = await supabase
      .from('sesiones_caja')
      .update({
 ...
 * @returns La sesión de caja actualizada
 */
export const forzarCierreCaja = async (sesionId: string, motivo: string): Promise<SesionCaja> => {
  try {
    const { data, error } = await supabase
      .from('sesiones_caja')
      .update({
 ...
 */
export const puedeAbrirCaja = async (usuarioId: string, cajaId: string): Promise<boolean> => {
  try {
