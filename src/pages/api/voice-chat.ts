...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {  
  ...
  try {  
    const { message, context, empresa_id } = req.body;  
...
PRODUCTOS CRÍTICOS:  
${context.inventario.productos_criticos.map((p: any) => `- ${p.nombre}: ${p.stock} unidades (mínimo: ${p.minimo})`).join('\n')}  
  
SUCURSALES:  
${context.sucursales.map((s: any) => `- ${s.nombre}: ${s.activa ? 'Activa' : 'Inactiva'}`).join('\n')}  
