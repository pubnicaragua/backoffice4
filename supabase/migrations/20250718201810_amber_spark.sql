/*
  # Fix JSON Error and Complete POS Integration System
  
  1. Fix JSON syntax error in payment providers
  2. Create complete POS integration tables
  3. Add sample data with correct JSON format
  4. Optimize queries with proper indexes
*/

-- Create payment providers table with correct JSON
...
CREATE TABLE IF NOT EXISTS pos_terminals (
  ...
  payment_providers uuid[] DEFAULT '{}',
  configuration jsonb DEFAULT '{}',
  created_at timestamptz DEFAULT now()
);
...
-- Insert payment providers with CORRECT JSON syntax
